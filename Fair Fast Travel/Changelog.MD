...
**2.20.0:**
- Fixed: backpack travel unavailable for Guide Travel type
- Fixed: object names instead of stash names shown in some circumstances
- Fixed: rare but possible crash when Anomaly leaves a null squad in `axr_companions.companion_squads`
- New context-sensitive NPE tutorial is offered when travel is blocked by a psy field
- Overall improved timed-prompt tutorial

**2.19.0:**
- Tutorial now uses my Timed Tutorial Prompt module to prompt the player before showing the tutorial, instead of interrupting gameplay
- Minor adjustments to travel logic
- Utility scripts updated to 20260122

**2.18.3:**
- Added another null-value sanity check
- Fix for campfire travel checks not working in some circumstances

**2.18.2:**
- Added null value checks in various places to avoid rare crashes that ought to be impossible
- Improved size and positioning in high resolutions for some MCM text header elements

**2.18.1:**
- Fix for location discovery spam

**2.18.0:**
- Updated utility scripts to 20250717, and overhauled/refactored using their recent features
- This also brings along many fixes to them from recent months, especially with text handling
- Added new NPE tutorial module, which plays the first time fast travel is unlocked
- Removed my old MCM monkeypatch script, as it is for a version of MCM and Anomaly that is no longer supported
- Minor adjustments to travel zone indicator script

**2.17.2:**
- Updated utility scripts to the 20250422 versions
